  ...
  return (
    <div className="prose prose-lg mx-auto max-w-[760px] text-text leading-[1.8]">
      <PortableText
        ...
        components={{
          block: {
            h2: ({ children }) => <h2 className="mt-10 mb-4 text-2xl font-bold text-text">{children}</h2>,
            h3: ({ children }) => <h3 className="mt-8 mb-3 text-xl font-bold text-text">{children}</h3>,
            normal: ({ children }) => <p className="mb-5 text-text-secondary">{children}</p>,
            blockquote: ({ children }) => (
              <blockquote className="border-l-4 border-primary pl-4 italic text-text-muted">{children}</blockquote>
            ),
          },
          marks: {
            link: ({ children, value }) => (
